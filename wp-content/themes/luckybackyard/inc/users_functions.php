...
function custom_register_user()
{
    ...
    $register_response = array(
        'registration_text' => 'Your account has been created successfully. Please proceed verification process',
        'lb_user_id' => $user_id,
        'verification_url' => home_url('/verification-page?lb_user_id=' . $user_id)
    );

    wp_send_json($register_response);
...
{
    // Check if start_date and end_date are set in $_POST
    if (isset($_POST['start_date']) && isset($_POST['end_date'])) {
        ...
        // Return error if start_date or end_date is not set
        wp_send_json_error('Error: Start date or end date is not set.');
    }
}
add_action('wp_ajax_calculate_days', 'calculate_days');
add_action('wp_ajax_nopriv_calculate_days', 'calculate_days');
...
function update_stripe_token()
{
    // Make sure to include Stripe library
    require_once('vendor/autoload.php');

    // Set your Stripe secret key
    \Stripe\Stripe::setApiKey('sk_test_5vT7VvxstxkE7CxNyfxNOFoQ');

    try {
        $formData = $_POST['formData'];
        parse_str($formData, $fields);
        extract($fields);

        // Create a PaymentIntent using Stripe SDK
        $payment_intent = \Stripe\PaymentIntent::create([
            'payment_method' => $payment_method_id,
            'amount' => $total_amount * 100, // Amount is in cents
            'currency' => 'usd',
            'confirmation_method' => 'manual',
            'confirm' => true,
            'return_url' => 'https://insitechstaging.com/demo/luckybackyard/customwp/request_to_book/',
        ]);

        // Return success response with payment intent information
        echo json_encode(['success' => true, 'msg' => 'Payment successful!', 'payment_intent' => $payment_intent]);
    } catch (Exception $e) {
...
add_action('wp_ajax_update_stripe_token', 'update_stripe_token');
add_action('wp_ajax_nopriv_update_stripe_token', 'update_stripe_token');
